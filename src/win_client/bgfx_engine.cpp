...
void BgfxEngine::draw() {
    double view_x_pos = g_player_location.position.x + sin(g_player_location.direction /* * GRAD_TO_RAD*/) * 10;
    double view_y_pos = g_player_location.position.y + cos(g_player_location.direction /* * GRAD_TO_RAD*/) * 10;
    float view_matrix[16];
    bx::mtxLookAt(view_matrix,
                  bx::Vec3(static_cast<float>(g_player_location.position.x), static_cast<float>(g_player_location.position.y), 1.6f),
