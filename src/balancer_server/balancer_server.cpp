...
BalancerServer::BalancerServer(const std::string& cfg_file_name_) :
    ...
    setApplicationType(Packet::EApplicationType::BalancerServer);

    CoordinateType global_bounding_box_start_x = 0;
    CoordinateType global_bounding_box_start_y = 0;
    CoordinateType global_bounding_box_size = NODE_SIZE_MAX;
    ...
    for (auto mac_address_it = node_server_mac_address.begin(); mac_address_it != node_server_mac_address.end(); ++mac_address_it) {
        ...
        available_node_servers.emplace(new_node_server_info->ip_address.to_bytes(), new_node_server_info);
    }
    ...
    if (NODE_SIZE_MIN > NODE_SIZE_MAX) {
        throw std::runtime_error("NODE_SIZE_MIN > NODE_SIZE_MAX");
    }
    if (global_bounding_box_size > NODE_SIZE_MAX) {
        throw std::runtime_error("global_bounding_box_size > MAXIMAL_NODE_SIZE");
    }
    while (global_bounding_box_size > NODE_SIZE_MIN) {
...
bool BalancerServer::start() {
    ...
    Logger::setLoggingToFile(log_to_file, "balancer_server_" + std::to_string(port_number) + ".log");
...
BalanceTree* BalancerServer::createNewBalanceNode(const Square2D& bounding_box, BalanceTree* parent) {
    IndexType new_node_token;
...
void BalancerServer::destroyBalanceNode(BalanceTree* node) {
    IndexType node_token = node->getToken();
    node->~BalanceTree();
...
void BalancerServer::startNode(NodeInfo& node_info, BalanceTree* balance_tree) {
    ...
    if (it == end_point_to_tree.end()) {
        end_point_to_tree.emplace(end_point, balance_tree);
    } else {
        assert(false);
...
#ifdef _DEBUG
    LOG_DEBUG << "user token " << packet->unit_location.unit_token;
#endif
    ...
    if (node_it != end_point_to_tree.end()) {
        ...
        if (found_node_info_it != available_node_servers.end()) {
            if (!found_node_info_it->second->power_on) {
                ...
                for (PortNumberType port_number = NODE_SERVER_PORT_NUMBER_BASE + 1; port_number < NODE_SERVER_PORT_NUMBER_BASE + found_node_info_it->second->max_process_count; ++port_number) {
...
bool BalancerServer::onRegisterProxy(size_t received_bytes) {
    ...
    auto find_it = id_to_proxy.find(packet->proxy_token);
    if (find_it == id_to_proxy.end()) {
        IndexType new_proxy_token = static_cast<IndexType>(id_to_proxy.size() + 1);
        answer->proxy_token = new_proxy_token;
        id_to_proxy.emplace(new_proxy_token, remote_end_point);
    } else {
        id_to_proxy[packet->proxy_token] = remote_end_point;
        answer->proxy_token = packet->proxy_token;
    }
...
bool BalancerServer::onMonitoringBalancerServerInfo(size_t received_bytes) {
    ...
    answer->global_bounding_box.start.x = global_bounding_box.start.x;
    answer->global_bounding_box.start.y = global_bounding_box.start.y;
    answer->global_bounding_box.size = global_bounding_box.size;
...
bool BalancerServer::onMonitoringBalanceTreeNeighborInfo(size_t received_bytes) {
    ...
    BalanceTree* found_tree = uuid_to_tree.find(packet->tree_node_token);
    if (found_tree) {
        found_tree->getMonitoringNeighborInfo(answer, packet->neighbor_cell.toCoordinate2D());
    } else {
        answer->success = false;
    }
    standardSend(answer);
...
bool BalancerServer::onMonitoringGetProxyCount(size_t received_bytes) {
    ...
    answer->proxy_count = static_cast<IndexType>(id_to_proxy.size());
    standardSend(answer);
    return true;
}
...
void BalancerServer::initAvailableNodes() {
    ...
    for (auto node_server_info : available_node_servers) {
        if (node_server_info.second->ip_address.is_loopback()) {
            for (PortNumberType port_number = NODE_SERVER_PORT_NUMBER_BASE; port_number < NODE_SERVER_PORT_NUMBER_BASE + node_server_info.second->max_process_count; ++port_number) {
                NodeInfo new_node_info;
