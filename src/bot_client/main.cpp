...
UnitLocationToken g_player_location;
...
void moveToTarget(UDPConnection& connection, double target_x_pos, double target_y_pos) {
    ...
    while (distance_to_target > 5.0) {
        // TODO:
        //Coordinate2D to_target_vector(target_x_pos - g_player_location.x_pos, target_y_pos - g_player_location.y_pos);
...
void program(int argc, char* argv[]) {
    ...
    PortNumberType server_port_number = 17012;
    std::list<double> control_point_x_positions;
    std::list<double> control_point_y_positions;
    g_player_location.position.x = 5000;
    g_player_location.position.y = 5000;
    ...
    config_reader.addParameter("bot_start_x_pos", g_player_location.position.x);
    config_reader.addParameter("bot_start_y_pos", g_player_location.position.y);
