...
void MainMonitorWindow::onConnect() {
    ...
    if (dialog.exec()) {
        connectedState();
        connection = new MonitorUDPConnection(host_name_edit.text(), static_cast<PortNumberType>(port_number_edit.value()), this);
    }
}
...
void MainMonitorWindow::onMonitoringBalancerServerInfoAnswer(QByteArray data) {
    ...
    if (data.size() >= Packet::getSize(answer)) {
        ...
        server_info->bounding_box = answer->global_bounding_box.toSquare2D();
            ...
            if (answer->success) {
                ...
                if (!tree_info->leaf_node) {
                    ...
                    for (tree_info->current_child_index_to_send = 0; tree_info->current_child_index_to_send < ChildLast; ++tree_info->current_child_index_to_send) {
                        IndexType child_token = tree_info->children[tree_info->current_child_index_to_send];
                        if (child_token) {
                ...
                while (!sent_new_request && !server_info->parent_stack.empty()) {
                    IndexType parent_token = server_info->parent_stack.back();
                    ...
                    if (parent_tree_info) {
                        ...
                        if (parent_tree_info->current_child_index_to_send < ChildLast) {
                            IndexType sibling_token = parent_tree_info->children[parent_tree_info->current_child_index_to_send];
...
void MainMonitorWindow::onMonitoringBalanceTreeNeighborInfoAnswer(QByteArray data) {
    ...
    if (data.size() >= Packet::getSize(answer)) {
        if (answer->success) {
            ...
            if (tree_info) {
                if (tree_info->leaf_node) {
                    std::pair<CoordinateType, CoordinateType> coordinate(answer->neighbor_cell.x, answer->neighbor_cell.y);
...
void MainMonitorWindow::onMonitoringGetProxyCountAnswer(QByteArray data) {
    ...
    if (data.size() >= Packet::getSize(answer)) {
        ...
        for (IndexType index = 1; index <= answer->proxy_count; ++index) {
        ...
        if (server_info->proxy_info_requests.empty()) {
            ...
            IndexType cur_proxy_index = server_info->proxy_info_requests.front();
...
void MainMonitorWindow::generateNeighborRequests(IndexType token) {
    if (isShowNeighbor()) {
