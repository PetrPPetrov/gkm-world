...
void BalancerMonitorWidget::paintEvent(QPaintEvent* event) {
    ...
    const CoordinateType cells_to_draw_width = static_cast<CoordinateType>(world_to_draw_width / CELL_SIZE) + 1;
    const CoordinateType cells_to_draw_height = static_cast<CoordinateType>(world_to_draw_height / CELL_SIZE) + 1;
    const CoordinateType cur_cell_x = static_cast<CoordinateType>(view_point_x / CELL_SIZE);
    const CoordinateType cur_cell_y = static_cast<CoordinateType>(view_point_y / CELL_SIZE);
    const CoordinateType start_cell_x = cur_cell_x - cells_to_draw_width / 2 - 1;
    const CoordinateType start_cell_y = cur_cell_y - cells_to_draw_height / 2 - 1;
    const CoordinateType end_cell_x = cur_cell_x + cells_to_draw_width / 2 + 1;
    const CoordinateType end_cell_y = cur_cell_y + cells_to_draw_height / 2 + 1;

    auto server_info = g_main_window->getServerInfo();
    if (server_info) {
        typedef boost::polygon::point_data<CoordinateType> Point;
        typedef boost::polygon::rectangle_data<CoordinateType> Rectangle;
        typedef boost::polygon::polygon_90_data<CoordinateType> Polygon90;
        typedef boost::polygon::polygon_90_set_data<CoordinateType> Polygon90Set;
        ...
        Square2D global_box = server_info->bounding_box;
            ...
            while (!parent_stack.empty()) {
                ...
                if (cur_node) {
                    if (cur_node->leaf_node) {
                        ...
                        Square2D node_box = cur_node->bounding_box;
                        ...
                        for (std::uint8_t i = ChildFirst; i < ChildLast; ++i) {
                            IndexType child_token = cur_node->children[i];
                            if (child_token) {
        ...
        if (g_main_window->isShowSelectedNode() && server_info->selected_node) {
            ...
            Square2D selection_box = server_info->selected_node->bounding_box;
            ...
            if (print_neighbors && g_main_window->isShowNeighbor()) {
                for (CoordinateType x = start_cell_x; x <= end_cell_x; ++x) {
                    for (CoordinateType y = start_cell_y; y <= end_cell_y; ++y) {
                        std::pair<CoordinateType, CoordinateType> coordinate(x, y);
