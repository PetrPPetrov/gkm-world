...
MonitorUDPConnection::MonitorUDPConnection(const QString& host_name, PortNumberType port_number, MainMonitorWindow* main_window_)
...
void MonitorUDPConnection::onAddProxyLog(unsigned proxy_index, QString ip_adddress, unsigned port_number) {
    ...
    if (find_it == proxies_info.end()) {
        ...
        proxy_server_info->application_type = Packet::EApplicationType::ProxyServer;
        proxy_server_info->server_token = static_cast<IndexType>(proxy_index);
        proxy_server_info->ip_address.setAddress(ip_adddress);
        proxy_server_info->port_number = static_cast<PortNumberType>(port_number);
...
void MonitorUDPConnection::onResolve(QHostInfo host_info) {
    ...
    for (auto& host_address : host_info.addresses()) {
        ...
        balancer_server_info->application_type = Packet::EApplicationType::BalancerServer;
