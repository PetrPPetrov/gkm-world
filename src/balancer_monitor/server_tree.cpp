...
ProxyItem::ProxyItem(IndexType proxy_index_)
    : TreeItem(QString("Proxy Server %1").arg(proxy_index_)) {
    proxy_index = proxy_index_;
}

IndexType ProxyItem::getProxyIndex() const {
    return proxy_index;
}
...
std::vector<TreeItem::Ptr> buildProxyPropertyList(const ServerInfo::Ptr& server_info, IndexType proxy_index) {
