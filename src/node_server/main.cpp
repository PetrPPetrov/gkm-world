...
int main(int argc, char** argv) {
    ...
    PortNumberType port_number = 17014;
    if (argc >= 2) {
        port_number = static_cast<PortNumberType>(std::stoi(argv[1]));
    }
    ...
    bool result = false;
    try {
        std::unique_ptr<NodeServer> node_server = std::make_unique<NodeServer>(port_number, cfg_file_name);
        std::unique_ptr<LogicThread> logic_thread = std::make_unique<LogicThread>(node_server->getNodeList());
        logic_thread->start();
