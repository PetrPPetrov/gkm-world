...
namespace Packet {
    ...
    struct ProtocolCoordinate2D {
        ...
        ProtocolCoordinate2D() = default;
        ProtocolCoordinate2D(const Coordinate2D& value) : x(value.x), y(value.y) {}
        Coordinate2D toCoordinate2D() const {
    ...
    struct ProtocolUnitLocation {
        ...
        ProtocolUnitLocation() = default;
        ProtocolUnitLocation(const UnitLocation& value) : position(value.position), direction(value.direction) {}
        UnitLocation toUnitLocation() const {
            return UnitLocation(position.toCoordinate2D(), direction);
        }
    };
    ...
    struct ProtocolUnitLocationToken : public ProtocolUnitLocation {
        ...
        ProtocolUnitLocationToken() = default;
        ProtocolUnitLocationToken(const UnitLocationToken& value) : ProtocolUnitLocation(value), unit_token(value.unit_token) {}
        UnitLocationToken toUnitLocationToken() const {
            return UnitLocationToken(toUnitLocation(), unit_token);
        }
    };
    ...
    struct ProtocolKeyboardState {
        ...
        ProtocolKeyboardState() = default;
        ProtocolKeyboardState(const KeyboardState& keyboard_state) {
            ...
            if (keyboard_state.right) {
                state |= RIGHT_KEY_MASK;
            }
        }
        KeyboardState toKeyboardState() const {
            KeyboardState result;
            if (state & UP_KEY_MASK) {
                result.up = true;
            }
            if (state & DOWN_KEY_MASK) {
                result.down = true;
            }
            if (state & LEFT_KEY_MASK) {
                result.left = true;
            }
            if (state & RIGHT_KEY_MASK) {
                result.right = true;
            }
            return result;
        }
    ...
    struct ProtocolSquare2D {
        ...
        CoordinateType size = 0;

        ProtocolSquare2D() = default;
        ProtocolSquare2D(const Square2D& square) : start(square.start), size(square.size) {}
        Square2D toSquare2D() const {
            return Square2D(start.toCoordinate2D(), size);
        }
    };
