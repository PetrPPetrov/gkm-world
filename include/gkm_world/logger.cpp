...
struct ThreadLogInfo
{
    ...
    Memory::FastIndexChain<Message> message_queue;
};
...
void InternalDumper::reportMessage(Packet::ESeverityType severity, const std::string& text, bool network_send)
{
    ...
    Memory::FastIndexChain<Message>& message_queue = fit->second->message_queue;
...
void InternalDumper::processNextQueuedMessage()
{
    ...
    const Memory::FastIndexChain<Message>& message_queue = cur_thread_info_it->second->message_queue;
...
void Logger::registerThread(std::thread::id thread_id)
{
    g_logger.registerThread(thread_id);
}
