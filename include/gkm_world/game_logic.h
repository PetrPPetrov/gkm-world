...
struct UnitLocation
{
    ...
    UnitLocation(Coordinate2D position_, double direction_)
        : position(position_), direction(direction_)
    {
    }
    UnitLocation(double x_position, double y_position, double direction_)
        : position(x_position, y_position), direction(direction_)
    {
    }
};
...
inline void gameStep(UnitLocation& unit_location, const KeyboardState& keyboard_state) noexcept
{
    ...
    double x_pos = toFloat(unit_location.position.x);
    double y_pos = toFloat(unit_location.position.y);
    ...
    unit_location.position.x = toCoordinateType(x_pos);
    unit_location.position.y = toCoordinateType(y_pos);
